...
PageVO<MaterialTypeTreeListVO> MaterialTypeTreeListService::listAll(const MaterialTypeItemQuery& query)
{
	...
	for (MaterialTypeTreeDO sub : result)
	{
		...
		if (sub.getHasChild() == "1") {
			...
			for (MaterialTypeTreeDO cb : result) {
				if (cb.getPid() == sub.getId()) {
					...
					if (sub.getIsEnabled() == 1) co.setIsEnabled_dictText(CharsetConvertHepler::ansiToUtf8("是"));
					else co.setIsEnabled_dictText(CharsetConvertHepler::ansiToUtf8("否"));
		...
		if(sub.getIsEnabled() == 1)vo.setIsEnabled_dictText(CharsetConvertHepler::ansiToUtf8("是"));
		else vo.setIsEnabled_dictText(CharsetConvertHepler::ansiToUtf8("否"));
